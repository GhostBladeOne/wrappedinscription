...
async function main() {
  ...
  const token = await  deployContract('TestERC20' , ['1000000000000000000000000']);

	console.log("token : %s", token.address)
  const txhash = '0xd893ca77b3122cb6c480da7f8a12cb82e19542076f5895f21446258dc473a7c2';
	const WrappedInscription = await deployContract('WrappedInscription', [token.address , txhash ,18])
